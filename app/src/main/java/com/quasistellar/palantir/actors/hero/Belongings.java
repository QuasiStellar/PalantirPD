...
import com.quasistellar.palantir.items.KindofMisc;
...
public class Belongings implements Iterable<Item> {
	...
	public void restart(int depth) {
		...
		for (Item item : backpack.items.toArray(new Item[0])) {
            ...
            } else if (item instanceof Stone5) {
				if (depth == 5) ((Stone5)item).charge = 0;
            ...
            } else if (item instanceof Diamond5) {
				if (depth == 5) ((Diamond5)item).charge = 0;
            } else if (!item.isEquipped(owner)) {
				item.detachAll(backpack);
			}
