	...
	protected void createItems() {
		...
		for (int i = 0; i < LENGTH; i++) {
			if (map[i]==Terrain.SOKOBAN_HEAP){
				drop(new Stone3.BlueStone(), i).type = Heap.Type.CHEST;
			}
		}

		drop(new Diamond3.BlueDiamond(), 25 + WIDTH * 4);
		 ...
		 addItemToGen(new Stone3.BlueStone() , 6, 30 + WIDTH * 23);
		 addItemToGen(new Stone3.BlueStone() , 7, 18 + WIDTH * 23);
		 addItemToGen(new Stone3.BlueStone() , 8, 26 + WIDTH * 2);
		 addItemToGen(new Stone3.BlueStone() , 9, 8 + WIDTH * 37);
		 addItemToGen(new Stone3.BlueStone() , 10, 10 + WIDTH * 37);
	}
