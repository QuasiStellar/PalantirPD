...
public class SokobanPerlLevel extends Level {
  ...
  public void addItemToGen(Item item, int arraypos, int pos) {
	...
	protected boolean build() {
		...
		map = SokobanLayouts2.SOKOBAN_PERL.clone();
		decorate();
...
//			mob.state = mob.SLEEPING;
	...
	protected void createSwitches(){
		...
		teleportspots[0] = 21 + WIDTH * 27;
		teleportspots[1] = 26 + WIDTH * 27;

		//spots where your portal switches are		
		portswitchspots[0] = 9 + WIDTH * 27;
		portswitchspots[1] = 4 + WIDTH * 27;

		//assign each switch to a portal		
		teleportassign[0] = 21 + WIDTH * 27;
		teleportassign[1] = 21 + WIDTH * 27;

		//assign each switch to a destination spot	
		destinationassign[0] = 15 + WIDTH * 29;
		destinationassign[1] = 25 + WIDTH * 26;

		//set the original destination of portals	
		destinationspots[0] = 0;
		destinationspots[1] = 15 + WIDTH * 27;
	...
	protected void createItems() {
		for (int i = 0; i < LENGTH; i++) {
			if (map[i]==Terrain.SOKOBAN_HEAP){
				drop(new Stone2.YellowStone(), i).type = Heap.Type.CHEST;
			}
		}
		 
		 addItemToGen(new IronKey(Dungeon.depth) , 0, 14 + WIDTH * 15);
		 addItemToGen(new IronKey(Dungeon.depth) , 1, 16 + WIDTH * 15);
		 addItemToGen(new IronKey(Dungeon.depth) , 2, 14 + WIDTH * 17);
		 addItemToGen(new IronKey(Dungeon.depth) , 3, 16 + WIDTH * 17);
		 
		 drop(new Diamond2.YellowDiamond(), 27 + WIDTH * 28);
	}
