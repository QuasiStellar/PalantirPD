...
import com.quasistellar.otiluke.items.stones.Stone1;
import com.quasistellar.otiluke.items.stones.Stone2;
import com.quasistellar.otiluke.items.stones.Stone3;
import com.quasistellar.otiluke.items.stones.Stone4;
import com.quasistellar.otiluke.items.stones.Diamond1;
import com.quasistellar.otiluke.items.stones.Diamond2;
import com.quasistellar.otiluke.items.stones.Diamond3;
import com.quasistellar.otiluke.items.stones.Diamond4;
...
public class Belongings implements Iterable<Item> {
	...
	public void restart(int depth) {
		...
		for (Item item : backpack.items.toArray(new Item[0])) {
			...
			} else if (item instanceof Stone1) {
				if (depth == 1) ((Stone1)item).charge = 0;
            } else if (item instanceof Stone2) {
			    if (depth == 2) ((Stone2)item).charge = 0;
            } else if (item instanceof Stone3) {
				if (depth == 3) ((Stone3)item).charge = 0;
            } else if (item instanceof Stone4) {
				if (depth == 4) ((Stone4)item).charge = 0;
            } else if (item instanceof Diamond1) {
				if (depth == 1) ((Diamond1)item).charge = 0;
            } else if (item instanceof Diamond2) {
				if (depth == 2) ((Diamond2)item).charge = 0;
            } else if (item instanceof Diamond3) {
				if (depth == 3) ((Diamond3)item).charge = 0;
            } else if (item instanceof Diamond4) {
				if (depth == 4) ((Diamond4)item).charge = 0;
            } else if (!item.isEquipped(owner)) {
				item.detachAll(backpack);
			}
